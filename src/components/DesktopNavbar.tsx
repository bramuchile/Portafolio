...
const DesktopNavbar = ({ onPageChange }: Props) => {
  ...
  return (  
    <AppBar position="absolute" color="transparent" elevation={0} sx={{ mt:0,m:0, top: 0, // Ajusta la distancia desde arriba
      right: -24, // Ajusta la distancia desde la derecha
      zIndex: 0, }}>
      <Toolbar sx={{ justifyContent: "flex-end" }}>
        <Box display="flex" gap={1} sx={{bgcolor:"#282829", borderRadius:"0 20px 0 20px", p:2,
              border: "1px solid rgba(255, 255, 255, 0.1)", // Borde blanco delgado
              }}>
          {menuItems.map((item) => (
            <Button
              key={item.key}
